...
const EXERCISES = [
  // Level 1: nominal sentence – “I am Amma.”
  {
    id: "nominal-1",
    type: "nominal",
    level: 1,
    englishPrompt: "I am Amma.",
    availableTiles: ["நான்", "அம்மா"],
    correctAnswers: [
      ["நான்", "அம்மா"], // நான் அம்மா
    ],
  },
  // Level 2: SOV – “I read the book.”
  {
    id: "sov-1",
    type: "sov",
    level: 2,
    englishPrompt: "I read the book.",
    availableTiles: ["நான்", "புத்தகத்தை", "படிக்கிறேன்"],
    correctAnswers: [
...
function isCorrectAnswer(ex, attempt) {
  return ex.correctAnswers.some((sol) => arraysEqual(sol, attempt));
}
...
const SentenceGame = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 gap-6" style={{ backgroundColor: "#f8fafc" }}>
      <div className="max-w-xl w-full bg-white shadow-md rounded-2xl p-6">
        <h1 className="text-2xl font-bold mb-2">
          Tamil Sentence Builder
        </h1>
        <p className="text-sm" style={{ color: "#64748b", marginBottom: "1rem" }}>
          Level {exercise.level} – {exercise.type === "nominal" ? "Nominal Sentence" : "SOV Sentence"}
        </p>

        <div className="mb-4">
          <p style={{ color: "#0f172a" }}>
            <span className="font-semibold">English:</span> {exercise.englishPrompt}
          </p>
        </div>
          ...
          >
            {slots.length === 0 && (
              <span className="text-xs" style={{ color: "#94a3b8" }}>
                Click the Tamil tiles below to add them here…
              </span>
            )}
            {slots.map((word, idx) => (
              <button
        ...
        {/* Display current Tamil sentence */}
